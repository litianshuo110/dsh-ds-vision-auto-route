...
import { VISION_ROUTE_SERVICE } from './index.ts'
import type { VisionRouteService } from './types.ts'

const PACKAGE_NAME = 'dsh-ds-vision-auto-route'
...
function validateRouteRecord(
  history: readonly SessionEvent[],
  event: SessionEvent<'llm-vision-route/route'>,
...
function validateSession(session: Session, fallback: { provider: string; model: string } | undefined, fail: InvariantFailure): void {
...
const install: InvariantInstaller = Object.assign((ctx: Context, fail: InvariantFailure) => {
  const service = ctx.get(VISION_ROUTE_SERVICE) as VisionRouteService | undefined
