...
## 与已发布 DeepSeek Harness 版本的兼容性

本插件挂在 agent 循环的 `agent/request` waterfall 上，其执行时机在**宿主的提示准入之后**。图片准入判断位于 Web 与 ACP 入口层，而非本插件内。

- 在**源码版 harness**（从 checkout 运行 `pnpm dsh web`）中，该准入门会先咨询本插件的 `llm-vision-route` 服务再接收图片输入，因此文本-only 主模型上的图片提示会被放行，并自动路由到视觉回退模型。
- 在**已发布的 npm 版**（全局安装 `@deepseek-ai/dsh`、运行 `dsh web`）中，准入门仍会对文本-only 模型拒绝图片提示——**发生在本插件能起作用之前**。仅安装本插件并不能改变该宿主行为，因此标准 npm 宿主上，文本-only 主模型**不会**自动路由图片，图片会被拒绝，你需要手动把模型切到视觉模型。

插件在源码版、或任何其准入会咨询 vision-route 服务的宿主上开箱即用（该服务感知的宿主准入预计在未来的官方版本中发布）。无论哪种方式，手动切换模型始终可用。
